...
function recognize_image(callback) {
  ...
  pr.on('message', function(m) {
    var img = new Image();
    img.src = 'output.jpg?v=' + Math.random();
    img.onload = function(){
    ...
    if (typeof(m.body) === 'string') {
      if (!errorState) {
        ...
        auto_login('always', function() {
          ...
          tips.textContent = '正在选课';
        });
      }
      return;
      
    }
    if (typeof(m.body.jxbh) === 'string') {
      ToastList.add(m.body.jxbh + ' 选课成功');
      var index = electList.indexOf(m.body.jxbh);
      if (index > -1) {
        electList.splice(index, 1);
      }
      tips.textContent = '选中一些课程，还在选课';
      return;
    }
...
function init() {
  ...
  auto_login = function(typ, callback) {
    ...
    c.getCaptchaImage(function() {
      recognize_image(function(text) {
        ...
        c.login(function cb(err, res, body) {
          ...
          if (res.statusCode === 500 && loginState !== 2) {
            if (body.indexOf('验证码') >= 0) {
              auto_login('always', callback);
            } else {
              if (typ !== 'first') {
